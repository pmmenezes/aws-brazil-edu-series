# AWS Brasil - Conteúdo da série de Webinars para Educação apresentandos em 2020

## Utilitários

- [VPC template](vpc-template.yaml)
- [Solução para teste de carga](https://aws.amazon.com/solutions/distributed-load-testing-on-aws/)
- [Solução para teste de carga com JMeter](https://github.com/gabrielmartinigit/jmeter-ec2)

## Plugins Moodle relacionados a serviços AWS

- [AWS SDK for PHP](https://moodle.org/plugins/local_aws)
- [Amazon S3 Bucket](https://moodle.org/plugins/repository_s3bucket)
- [Amazon S3 Links](https://moodle.org/plugins/repository_s3links)
- [ElasticSearch](https://moodle.org/plugins/search_elastic)
- [Object Storage File System](https://moodle.org/plugins/tool_objectfs)
- [Streaming RTMP](https://moodle.org/plugins/filter_rtmp)
- [Document Converter](https://moodle.org/plugins/fileconverter_librelambda)
- [CloudFront URL Signer](https://moodle.org/plugins/filter_cloudfront_signurl)
- [DynamoDB Sessions](https://moodle.org/plugins/local_session_dynamodb)
